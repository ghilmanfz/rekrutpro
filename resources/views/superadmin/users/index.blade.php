    ...
    <main style="margin-left: 256px;">
        ...
        <div class="bg-white border-b px-8 py-4 flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Kelola pengguna internal sistem</p>
            </div>
        ...
        <div class="p-8">
            ...
            <div class="mb-6 flex justify-between items-center">
                ...
                <a href="{{ route('superadmin.users.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg flex items-center gap-2 transition-colors">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex gap-2">
                                    <a href="{{ route('superadmin.users.edit', $user) }}" class="text-blue-600 hover:text-blue-900" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <form action="{{ route('superadmin.users.toggle-status', $user) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="text-yellow-600 hover:text-yellow-900" title="{{ $user->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if($user->id !== auth()->id())
                                    <form action="{{ route('superadmin.users.destroy', $user) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus pengguna ini?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-4">Tidak ada data pengguna</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
